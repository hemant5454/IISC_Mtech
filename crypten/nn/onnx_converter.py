...
def from_onnx(onnx_string_or_file):
    ...
    op_type = "Clip"
    # op_type = "LSTM"
    # op_list = [node.op_type for node in onnx_model.graph.node]
    clip_nodes = [node for node in onnx_model.graph.node if node.op_type == op_type]
    ## Pop the element
    for i in range(len(clip_nodes)):
        clip_nodes[i].input[2] = "max_v"
    Y = helper.make_tensor('max_v', TensorProto.FLOAT, [], [10000000000000.])
...
ONNX_TO_CRYPTEN = {
    ...
    "LSTM": module.LSTM,
    "ReduceMax": module.ReduceMax,
    "Tile": module.Tile,
    "LeakyRelu": module.LeakyRelu,
    "Greater": module.Greater,
    "Split": module.Split,
    "Elu": module.Elu,
    "ScatterElements": module.ScatterElements,
}
