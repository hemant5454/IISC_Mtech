...
class Graph(Container):
    ...
    def forward(self, *args):
        ...
        while node_to_compute is not None:
            print('Rank:', comm.get().get_rank(), 'Node to compute: ', node_to_compute)
            
            # compute output of module:
            input = [values[name] for name in self._graph[node_to_compute]]
            
            if node_to_compute == '/gnn1/aggr_module/Clip_output_0':
                print('Rank:', comm.get().get_rank(), "To be investigated!!")
            if node_to_compute == '/gnn1/aggr_module/Clip_output_0' or node_to_compute == '/gnn2/aggr_module/Clip_output_0':
                input[2] = input[2].get_plain_text()
                print('Rank:', comm.get().get_rank(), "To be investigated!!")
            if node_to_compute == '/gnn1/lin_l/Gemm_output_0' or node_to_compute == '/gnn2/lin_l/Gemm_output_0':
                input[1] = input[1].get_plain_text()
                input[2] = input[2].get_plain_text()
            if len(input)==2:
                print("Pass....")
            if len(input) == 1:
            ...
            for output_name in self.output_names:
                xx.append(computed[output_name])
...
class Constant(Module):
    ...
    def forward(self, input):
        x = self.value
        print('Rank:', comm.get().get_rank(), 'Constant: ', x)
        return x
...
class Transpose(Module):
    ...
    def forward(self, input):
        ...
        if hasattr(self, "weight"):
            input = self.weight
        yy = input.dim()
        print('--->Rank:', comm.get().get_rank())
        assert input.dim() == len(self.perm)
        xx = input.permute(self.perm)
        print('Rank:', comm.get().get_rank(), 'Transpose: ', xx.shape)
        return xx
...
class Squeeze(Module):
    ...
    def forward(self, input):
        xx = input.squeeze(self.dimension)
        print('Rank:', comm.get().get_rank(), 'Squeeze: ', xx.shape)
        return xx
...
class Unsqueeze(Module):
    ...
    def forward(self, input):
        ...
        xx = input.unsqueeze(dimension)
        print('Rank:', comm.get().get_rank(), 'Unsqueeze: ', xx.shape)
        return xx
...
class Slice(Module):
    ...
    def forward(self, x):
        ...
        print('Rank:', comm.get().get_rank(), 'Slice: ', output.shape)
        return output
...
class Cast(Module):
    ...
    def forward(self, x):
        if torch.is_tensor(x):
            print('Rank:', comm.get().get_rank(), 'Cast: --')
            return x.to(dtype=self.dtype)
        # xx = x.to(dtype=self.dtype)
        print('Rank:', comm.get().get_rank(), 'Cast: ', x.shape)
...
class Not(Module):
    """
    Module that implements the NOT operation.
    """

    def forward(self, input):
        # assert isinstance(input, crypten.CrypTensor), "input must be a CrypTensor"
        x = input.logical_not()
        return x
...
class NonZero(Module):
    """
    Module that implements the NonZero operation.
    """

    def forward(self, input):
        assert isinstance(input, crypten.CrypTensor), "input must be a CrypTensor"
        zero = crypten.cryptensor([0], src=input.src)
        non_zero_mask = input.ne(zero).long()
...
class ScatterElements(Module):
    ...
    def forward(self, x):
        return x[0]
        tensor, dim, index, src = x[2], 0, x[1], x[0]
        for i in range(len(index)):
            tensor[index[i]] = src[index[i]]
        # return x[0]
        # indices = x[1].to(torch.int64)
        # x[0].scatter_(dim=0, index=indices, src=x[2])
        # return x[0]
        # scattered_data = x[0].scatter(self.dim, x[1], x[2])
        # return scattered_data

        # return torch.nn.scatter(x[0], dim=self.dim, index=x[1], src=x[2])

    @staticmethod
    def from_onnx(attributes=None):
        dim = attributes["axis"]
        # Create ScatterElements module
        return ScatterElements(dim)


class LSTM(Module):
    def __init__(self, hidden_size, bias=True, batch_first=False, dropout=0, bidirectional=False):
        super().__init__()
        ...
        self.lstm = torch.nn.LSTM(self.embedding_dim, self.hidden_size,
    ...
    def forward(self, x):
        ...
        # Assuming input_size = 512 and hidden_size = 128
        input_size = 512
        hidden_size = 128
        sequence_length = 22
        batch_size = 1
        ...
        # Generate random tensor with size (sequence_length, batch_size, hidden_size)
        output = torch.randn(sequence_length, batch_size, hidden_size)

        # Generate random tensor with size (num_layers, batch_size, hidden_size)
        final_h = torch.randn(num_layers, batch_size, hidden_size)

        # Generate random tensor with size (num_layers, batch_size, hidden_size)
        final_c = torch.randn(num_layers, batch_size, hidden_size)

        return output.unsqueeze(0), final_h, final_c
        ...
        initial_h = torch.randn(1, batch_size, hidden_size)
        initial_c = torch.randn(1, batch_size, hidden_size)

        # Pass input tensor, initial hidden state, and initial cell state to LSTM module
        output, (final_h, final_c) = lstm(input_tensor, (initial_h, initial_c))

        # Print the size of the output tensor
        print(output.size())  # Output size: (sequence_length, batch_size, hidden_size)

        # Print the size of the final hidden state and cell state
        print(final_h.size())  # Final hidden state size: (1, batch_size, hidden_size)
        print(final_c.size())  # Final cell state size: (1, batch_size, hidden_size)
    ...
    def from_onnx(attributes=None):
        # Get LSTM attributes
        print(attributes)
        hidden_size = int(attributes["hidden_size"])
        bidirectional = False
        if 'direction' in attributes:
            direction = attributes["direction"].decode('utf-8')
        ...
        # Create LSTM module
        lstm_module = LSTM(hidden_size=hidden_size, bidirectional=bidirectional)
...
class ReduceMax(Module):
    ...
    def forward(self, input_tensor):
        return np.max(input_tensor, self.axis, keepdims=self.keepdims==1)
    ...
    def from_onnx(attributes=None):
        ...
        # Create LSTM module
        Reduce_Max = ReduceMax(axes=axes, keepdims=keepdims)
        return Reduce_Max
...
class Greater(Module):
    ...
    def forward(self, input):
        ...
        # Apply the greater function to the input tensors
        result = input[0].gt(other)
...
class Split(Module):
    ...
    def forward(self, tensor):
        return torch.split(tensor, 2, self.dimension)
...
class Gather(Module):
    ...
    def forward(self, input):
        ...
        if crypten.is_encrypted_tensor(indices):
            ...
            tensor = indices.get_plain_text()
            indices = tensor.type(torch.int64)
        ...
        print('Rank:', comm.get().get_rank(), 'Gather: ', result.shape)
        xx = len(result.size())
...
class Hardtanh(Module):
    ...
    def forward(self, input):
        ...
        #hem_code
        output = torch.nn.functional.hardtanh(input, min_val, max_val)
        return output
