...
@register_function("leaky_relu")
class AutogradLeakyReLU(AutogradFunction):
    @staticmethod
    def forward(ctx, input, negative_slope=0.01):
        mask = input.gt(0.0)
        output = input.mul(mask) + negative_slope * input.mul(~mask)
        ctx.save_for_backward(mask)
    ...
    def backward(ctx, grad_output):
        ...
        grad_input[~mask] *= negative_slope
        return grad_input
