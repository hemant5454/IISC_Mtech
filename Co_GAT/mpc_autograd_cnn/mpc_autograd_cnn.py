...
sys.path.append('/data/home/hemantmishra/examples/CrypTen/Co_GAT')
...
def run_mpc_autograd_cnn(
    ...
    f.close()
    input_h, tensor_size, adj, adj_full, adj_re = data_conc(all_data)
    aa = input_h[0]
    bb = tensor_size[0]
    cc = adj[0]
    dd = adj_full[0]
    ...
    if rank == 1:
        ...
        x_bob = torch.empty(label.size())

    # encrypt
    # x_reduced = crypten.cryptensor(x_alice, src=0)
    y_reduced = crypten.cryptensor(x_bob, src=1)
    a_reduced = crypten.cryptensor(a, src=1)
    b_reduced = crypten.cryptensor(b, src=1)
    c_reduced = crypten.cryptensor(b, src=0)
    d_reduced = crypten.cryptensor(b, src=0)
    ...
    onnx_name = "model_onnx_new.onnx"
    # dummy_size = (5,34,60)
    # dummy_input = torch.zeros(dummy_size)

    # pytorch_to_onnx(model, input_h[0], tensor_size[0], adj[0], adj_full[0], adj_re[0], onnx_name)
    torch.onnx.export(model, (input_h[0], adj[0], adj_full[0], adj_re[0]), onnx_name)
    print("ONNX model completed!!")
    crypten_model = onnx_to_crypten(model, "/data/home/hemantmishra/examples/CrypTen/model_onnx_new.onnx")
    crypten_model.train()
    ...
    # encrypted training
    train_encrypted(
        a_reduced, tensor_size, b_reduced, c_reduced, d_reduced, y_reduced, crypten_model, args
    )
...
def train_encrypted(
    ...
    for epoch in range(args.num_epoch):
        ...
        for j in range(0, num_samples_train, args.batch_size):
            ...
            y_train = y_encrypted[j]
            y_train.requires_grad = True
            # y_train = crypten.cryptensor(y_one_hot, requires_grad=True)

            # perform forward pass:
            output = encrypted_model(x_train_1, x_train_2, x_train_3, x_train_4)
